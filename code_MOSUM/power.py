...
def MOSUM(X):
    ...
    for j in range(k, n - k):
        Z_j = (np.mean(X[:, j - k: j]) - np.mean(X[:, j: j + k])) / np.sqrt(2 * k)
...
T_quantile4 = np.percentile(quantile4, alpha)
...
for dlt in delta:
    ...
    num4 = sum(i <= T_quantile4 for i in data3) / epoch
    type2_1 = np.append(type2_1, num1)
    type2_2 = np.append(type2_2, num2)
    type2_3 = np.append(type2_3, num3)
    type2_4 = np.append(type2_4, num4)

type2 = [type2_1, type2_2, type2_3, type2_4]
np.savetxt('Power_p_{}_m_{}_sd.csv'.format(p,m), type2, delimiter=',', fmt='%4f')
